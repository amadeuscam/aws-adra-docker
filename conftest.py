...
def init_data(db):
    ...
    persona = Persona.objects.create(nombre_apellido="Florin salam", fecha_nacimiento=datetime.now(tz=timezone.utc),
                                     numero_adra=1,
                                     nacionalidad="española", domicilio="calle /", ciudad="madrid",
                                     telefono=604150313, modificado_por_id=user.pk, sexo='hombre', domingo='1')
...
def hijo_pentesco1(db, new_hijo_factory, init_data):
    user, persona, _ = init_data
    return new_hijo_factory(parentesco="esposa", modificado_por_id=user.pk, persona_id=persona.pk)


@pytest.fixture
def hijo_pentesco2(db, new_hijo_factory, init_data):
    user, persona, _ = init_data
    return new_hijo_factory(parentesco="esposo", modificado_por_id=user.pk, persona_id=persona.pk)


@pytest.fixture
def hijo_pentesco3(db, new_hijo_factory, init_data):
    user, persona, _ = init_data
    return new_hijo_factory(parentesco="hijo", modificado_por_id=user.pk, persona_id=persona.pk)


@pytest.fixture
def hijo_pentesco4(db, new_hijo_factory, init_data):
    user, persona, _ = init_data
    return new_hijo_factory(parentesco="hija", modificado_por_id=user.pk, persona_id=persona.pk)
...
def profile(db, init_data):
    user, _, _ = init_data
    prof = Profile.objects.create(user_id=user.pk, date_of_birth=date(1990, 12, 5))
    return prof
